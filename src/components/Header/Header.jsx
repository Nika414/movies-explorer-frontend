...
export default function Header({ loggedIn, location }) {
  return (
    <header className={`header ${location !== routeName.main ? 'header_location_authorized' : ''}`}>
      <Link className={`${(location === routeName.register || location === routeName.login) ? 'header__logo-link' : ''}`} to={routeName.main}>
